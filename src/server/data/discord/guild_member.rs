...
impl<'a> DiscordGuildMemberRepository<'a> {
    ...
    /// Deletes all members for a specific guild
    ///
    /// Removes all member records for a guild. Used when the bot leaves a guild
    /// or when performing a full resync.
    ///
    /// # Arguments
    /// - `guild_id`: Discord guild ID
    ///
    /// # Returns
    /// - `Ok(())`: All member records successfully deleted
    /// - `Err(DbErr)`: Database error during deletion
    pub async fn delete_all_by_guild(&self, guild_id: u64) -> Result<(), DbErr> {
        let guild_id_str = guild_id.to_string();
        entity::prelude::DiscordGuildMember::delete_many()
            .filter(entity::discord_guild_member::Column::GuildId.eq(guild_id_str.as_str()))
            .exec(self.db)
    ...
    /// Gets all logged-in users (with app accounts) for a specific guild
    ///
    /// Retrieves member information for users who have both:
    /// 1. A member record in this guild
    /// 2. A user account in the application (logged in via OAuth)
    ///
    /// # Arguments
    /// - `guild_id`: Discord guild ID
    ///
    /// # Returns
    /// - `Ok(Vec<(User, DiscordGuildMember)>)`: Vector of tuples with user and member info
    /// - `Err(DbErr)`: Database error during query
    pub async fn get_logged_in_members(
        &self,
        guild_id: u64,
    ) -> Result<Vec<(entity::user::Model, entity::discord_guild_member::Model)>, DbErr> {
        let members = self.get_members_by_guild(guild_id).await?;
        ...
        // Get users who have application accounts
        let users = entity::prelude::User::find()
            .filter(entity::user::Column::DiscordId.is_in(user_ids))
            .all(self.db)
            .await?;

        // Match users with their member records
        let mut results = Vec::new();
        for user in users {
            if let Some(member) = members.iter().find(|m| m.user_id == user.discord_id) {
                results.push((user, member.clone()));
            }
        }
